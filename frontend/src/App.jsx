...
{/* 依赖组件：AppLayout, react-router-dom */}
{/* 修改注意：新增页面时在此添加Route，公开页面(login)不走AppLayout */}
import { BrowserRouter, Routes, Route, Navigate } from 'react-router-dom';
import AppLayout from './components/AppLayout';

// 功能：临时占位页——T19-T32实现各页面后逐一替换
function PlaceholderPage({ title }) {
  return (
    <div id="placeholder-page-area" style={{
      ...
      <h2 style={{ color: '#8C8C8C' }}>{title} — 待实现 (T19-T32)</h2>
    </div>
  );
}
  ...
  return (
    <BrowserRouter>
      <Routes>
        ...
        <Route path="/login" element={<PlaceholderPage title="登录页" />} />
